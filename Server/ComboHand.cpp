...
{
	Straight conditieChinta;
	Flush conditieCuloare;

	return (conditieChinta.check(Pachet, carti) == true && conditieCuloare.check(Pachet, carti) == true);
}
...
bool RoyalFlush::check(const std::vector<_Carte>& Pachet, std::vector<_Carte> carti) const
{
	...
	if(conditieCuloare.check(Pachet, carti) == true){
		carti.insert(carti.end( ), Pachet.begin( ), Pachet.end( ));
